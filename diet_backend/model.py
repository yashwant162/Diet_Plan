...
# Assuming prep_data and pipeline are already trained
scaler.fit(prep_data)
neigh.fit(prep_data)

class RecipeRecommendationResource:
    def on_post(self, req, resp):
        """Handles POST requests for food recommendations."""
        try:
            raw_data = req.media
            user_input = np.array(raw_data['nutritional_info']).reshape(1, -1)
        except KeyError:
            resp.status = falcon.HTTP_400
            resp.media = {"error": "Invalid input format"}
            return
        ...
        recommendations = extracted_data.iloc[recommendations_idx]
